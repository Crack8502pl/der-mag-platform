                      ...
                      try {
                        const cabinetSubsystemTask = await this.taskService.createTask({
                          ...
                          metadata: {
                            ...
                            cabinetType: taskMeta.cabinetType,
                            autoGenerated: true,
      ...
      for (const taskData of tasks) {
        try {
          ...
          try {
            ...
            const taskMeta = taskData.metadata || {};
            const effectiveTaskType = taskData.type || 'GENERIC';
            const shouldGenerateCabinet =
              !!taskMeta.cabinetType &&
              requiresCabinetCompletion(effectiveTaskType) &&
              (taskMeta.generateCabinetCompletion ?? true);
            if (shouldGenerateCabinet) {
              try {
                const cabinetTaskName = `Kompletacja szafy - ${effectiveTaskType}`;
                const cabinetSubsystemTask = await this.taskService.createTask({
                  subsystemId: parseInt(id),
                  taskName: cabinetTaskName,
                  taskType: 'KOMPLETACJA_SZAF',
                  metadata: {
                    sourceTaskNumber: subsystemTask.taskNumber,
                    sourceTaskType: effectiveTaskType,
                    cabinetType: taskMeta.cabinetType,
                    autoGenerated: true,
                    generatedAt: new Date().toISOString()
                  }
                ...
                const savedCabinetMainTask = await taskRepository.save(cabinetMainTask);
                createdMainTasks.push(savedCabinetMainTask);

                serverLogger.info(
                  `✅ Automatycznie utworzono zadanie KOMPLETACJA_SZAF (${cabinetSubsystemTask.taskNumber}) dla ${subsystemTask.taskNumber} (${effectiveTaskType})`
                );
              } catch (cabinetError) {
                serverLogger.error(
                  `❌ Błąd tworzenia automatycznego zadania KOMPLETACJA_SZAF dla ${subsystemTask.taskNumber}:`,
