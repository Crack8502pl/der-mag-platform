...
const TemplatesTab: React.FC<{ canCreate: boolean; canUpdate: boolean; canDelete: boolean }> = ({ canCreate, canUpdate, canDelete }) => {
  const [templateSubTab, setTemplateSubTab] = useState<'bom-items' | 'recorders' | 'disks'>('bom-items');
  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: '20px' }}>
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)' }}>
          <button
            className={`btn ${templateSubTab === 'bom-items' ? 'btn-primary' : 'btn-secondary'}`}
            onClick={() => setTemplateSubTab('bom-items')}
            style={{
              borderRadius: '0',
              borderBottom: templateSubTab === 'bom-items' ? '2px solid var(--primary-color)' : 'none',
              flex: 1
            }}
          >
            📄 Pozycje BOM
          ...
          <button
            className={`btn ${templateSubTab === 'recorders' ? 'btn-primary' : 'btn-secondary'}`}
            onClick={() => setTemplateSubTab('recorders')}
            style={{
              borderRadius: '0',
              borderBottom: templateSubTab === 'recorders' ? '2px solid var(--primary-color)' : 'none',
              flex: 1
            }}
          >
            🖥️ Rejestratory
          ...
          <button
            className={`btn ${templateSubTab === 'disks' ? 'btn-primary' : 'btn-secondary'}`}
            onClick={() => setTemplateSubTab('disks')}
            style={{
              borderRadius: '0',
              borderBottom: templateSubTab === 'disks' ? '2px solid var(--primary-color)' : 'none',
              flex: 1
            }}
          >
            💾 Dyski
