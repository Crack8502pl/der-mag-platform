...
const CarsSyncSection: React.FC = () => {
  ...
  const loadStatus = useCallback(async () => {
    ...
    setStatusError('');
    try {
      const data = await symfoniaSyncService.getCarsStatus();
      setStatus(data);
    } catch (err: any) {
      setStatusError(err?.response?.data?.message || 'Błąd podczas pobierania statusu');
    } finally {
  ...
  const loadHistory = useCallback(async () => {
    ...
    setHistoryError('');
    try {
      const data = await symfoniaSyncService.getCarsHistory(10);
      setHistory(data);
    } catch (err: any) {
      setHistoryError(err?.response?.data?.message || 'Błąd podczas pobierania historii');
    } finally {
  ...
  const handleSync = async () => {
    ...
    setLastResult(null);
    try {
      const result = await symfoniaSyncService.carsSync();
      setLastResult(result);
      await loadStatus();
      await loadHistory();
    } catch (err: any) {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <div className="card-header">
          <h2 className="card-title">Samochody — Synchronizacja</h2>
        </div>
        <div className="card-body">
          <div style={{ marginBottom: '1rem' }}>
            <button
              ...
              title="Natychmiastowa synchronizacja samochodów z Symfonii"
            >
              {syncLoading ? '⏳ Synchronizuję...' : '🔄 Odśwież'}
            </button>
          </div>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: 0 }}>
            🚗 <strong>Synchronizacja samochodów</strong> — pobiera wpisy z Symfonii w formacie &quot;S00XXX Samochód REJESTRACJA&quot;.
            Pozostałe wpisy są ignorowane. Archiwizuje samochody usunięte z Symfonii.
          ...
          <div className="card-body">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
              {[
                ...
                { label: 'Zaktualizowano', value: lastResult.stats.updated },
                { label: 'Pominięto', value: lastResult.stats.skipped },
                { label: 'Zarchiwizowano', value: lastResult.stats.archived ?? 0 },
                { label: 'Błędy', value: lastResult.stats.errors },
              ].map(({ label, value }) => (
                <div key={label} style={{ padding: '0.75rem', background: 'var(--bg-secondary, #f8f9fa)', borderRadius: 6, textAlign: 'center' }}>
                  <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>{value}</div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="card-body">
          ...
          {status ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Ostatnia synchronizacja</div>
                <div style={{ fontWeight: 600 }}>{formatDate(status.lastFullSync)}</div>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Następna automatyczna</div>
              ...
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>CRON (auto co 12h)</div>
                <div style={{ fontWeight: 600 }}>
                  {status.cronEnabled ? (
                    <span style={{ color: '#155724' }}>✅ Aktywny</span>
                  ) : (
                ...
                <tbody>
                  {history.map((h) => (
                    <tr key={h.id}>
                      ...
                      <td>{h.stats.created ?? '—'}</td>
                      <td>{h.stats.updated ?? '—'}</td>
                      <td>{h.stats.skipped ?? '—'}</td>
                      <td>{h.stats.archived ?? '—'}</td>
                      <td>{h.stats.errors ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
